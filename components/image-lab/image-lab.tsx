...
export function ImageLab() {
  const [family, setFamily] = useState<FilterFamily>('glyphs');
  const [settingsByPhoto, setSettingsByPhoto] = useState<
    Record<string, Record<FilterFamily, FilterSettings>>
  >(() =>
    Object.fromEntries(
      photos.map((photo) => [treatmentKey(photo.id), photoDefaults(photo.id)]),
    ),
  );
  const [photoId, setPhotoId] = useState<string>(photos[0].id);
  const [view, setView] = useState<'filtered' | 'original' | 'compare'>(
    'filtered',
  );
  ...
  const [renderedImage, setRenderedImage] = useState<HTMLImageElement | null>(
    null,
  );
  ...
  const error =
    failure?.src === photo.src && (!failure.family || failure.family === family)
      ? failure.message
      : '';
  ...
  function applyLook(look: SavedLook) {
    ...
    setSettingsByPhoto((current) => ({
      ...current,
      [key]: { ...current[key], [look.family]: look.settings },
    }));
    setView('filtered');
  }
  ...
  useEffect(() => {
    ...
    const img = new Image();
    img.onload = () => {
      if (!cancelled) setLoadedImage({ src: photo.src, image: img });
    };
    img.onerror = () => {
      if (!cancelled)
        setFailure({
          src: photo.src,
          message: 'This photograph could not load. Choose another below.',
        });
    };
    img.src = photo.src;
    return () => {
    ...
    const frame = requestAnimationFrame(() => {
      try {
        if (canvasRef.current) {
          renderGlyphs(canvasRef.current, image, deferred);
        ...
        setFailure({
          ...
          family: 'glyphs',
          message:
            'The glyph preview is unavailable in this browser. Try Print instead.',
        });
      }
    });
  ...
  function update(
    ...
    value: number | FilterSettings['type'],
  ) {
    setSettingsByPhoto((current) => ({
      ...current,
      [currentTreatment]: {
        ...current[currentTreatment],
        [family]: { ...current[currentTreatment][family], [key]: value },
      },
    }));
    setNotice('');
  }
  ...
  function getLook(): SavedLook {
    return { version: 1, photo: photo.id, family, settings };
  }
  ...
  function saveLook() {
    try {
      ...
      setNotice(
        'Browser storage is unavailable. Use Copy settings to keep this look.',
      );
    }
  }
  ...
  async function copySettings() {
    try {
      await navigator.clipboard.writeText(JSON.stringify(getLook(), null, 2));
      setNotice('Settings copied. Paste them into our conversation.');
    } catch {
      setNotice(
        'Copy is unavailable here. Select the settings below and copy them.',
      );
    }
  }
  ...
  async function download() {
    ...
    try {
      if (family === 'glyphs') {
        source = exportGlyphArtwork(
          image,
          settings,
          photo.id === 'mountaineering-mobile' ? 1536 : 3840,
        );
      } else {
        const mount = printRef.current?.paperShaderMount;
        if (!mount) {
          setNotice('Wait for the preview, then try again.');
          setExporting(false);
          return;
        }
      ...
      setNotice('The image could not export. Please try again.');
      setExporting(false);
      return;
    }
    ...
    ctx.globalAlpha = family === 'glyphs' ? 1 : settings.strength / 100;
    ...
    <main className={styles.lab}>
      <header className={styles.header}>
        <span className={styles.wordmark}>
          mihiir <span>/ image lab</span>
        </span>
        <a
          href="https://app.paper.design/file/01M24PYFVR42B77SGG89HQ1NXD/2-0"
          target="_blank"
          rel="noreferrer"
        >
      ...
      <div className={styles.workspace}>
        ...
        >
          <div className={styles.previewToolbar}>
            ...
            <div className={styles.viewSwitch} aria-label="Preview mode">
              {(['filtered', 'original', 'compare'] as const).map((mode) => (
                <button
                  key={mode}
                  type="button"
          ...
          <div
            className={`${styles.imageStage} ${photo.id === 'mountaineering-mobile' ? styles.portrait : ''}`}
          ...
          >
            ...
            >
              {family === 'glyphs' ? (
                webgl && image ? (
                  ...
                  <canvas ref={canvasRef} className={styles.glyphCanvas} />
                )
              ) : webgl && image ? (
                <HalftoneCmyk
                  ref={printRef}
                  ...
                  colorY="#ffff00"
                  colorK="#000000"
                  size={deferred.size / 100}
                  gridNoise={deferred.noise / 100}
                  softness={deferred.softness / 100}
                  type={deferred.type}
                  contrast={deferred.contrast / 100}
                  grainOverlay={deferred.grain / 100}
                  grainMixer={deferred.grain / 500}
                  grainSize={0.15}
                  floodC={0}
                  floodM={0}
                  floodY={0}
            ...
            {view === 'compare' && (
              <>
                ...
                <img
                  src={photo.src}
                  alt=""
                  className={styles.originalReveal}
                  style={{ clipPath: `inset(0 ${100 - reveal}% 0 0)` }}
                />
                <div className={styles.divider} style={{ left: `${reveal}%` }}>
                  <span aria-hidden="true">↔</span>
                </div>
              </>
            )}
            {(!image || (!ready && family === 'glyphs')) && !error && (
              <span className={styles.loading}>Preparing photograph…</span>
            )}
            {error && (
            ...
            {family === 'print' && webgl === false && (
              <p className={styles.previewError} role="alert">
                Print needs WebGL. Glyphs works in this browser.
              </p>
            )}
            {photo.id.startsWith('mountaineering') && (
              <PhotoMemory
                key={photo.id}
                mobile={photo.id === 'mountaineering-mobile'}
              />
            )}
          </div>
          {view === 'compare' && (
            <div className={styles.compareControl}>
              <span>Original</span>
              <input
                aria-label="Original photograph revealed"
                type="range"
                min="0"
                max="100"
                value={reveal}
          ...
          <div className={styles.photos} aria-label="Choose a photograph">
            {photos.map((p) => (
              ...
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={p.src} alt="" width="96" height="64" />
                <span>{p.name}</span>
              </button>
            ))}
          </div>
          <p className={styles.photoNote}>
            Photographs from my archive, with scenery expanded using image
            generation. Tune the texture on top.
          </p>
        </section>
        <aside className={styles.controls} aria-label="Filter controls">
          <div className={styles.familySwitch}>
            <button
              type="button"
              aria-pressed={family === 'glyphs'}
              onClick={() => {
                setFamily('glyphs');
                setNotice('');
              }}
            >
              <span aria-hidden="true">Aa</span> Glyphs
            </button>
            <button
              type="button"
              aria-pressed={family === 'print'}
              onClick={() => {
                setFamily('print');
                setNotice('');
              }}
          ...
          <p className={styles.familyHint}>
            {family === 'glyphs'
              ? 'Characters sampled from the colors in your photograph.'
              : 'Paper’s CMYK filter, from the reference you shared.'}
          </p>
          <Slider
            label={family === 'glyphs' ? 'Glyph size' : 'Dot size'}
            value={settings.size}
          ...
          <details className={styles.more}>
            ...
            <textarea
              aria-label="Current filter settings"
              readOnly
              value={JSON.stringify(getLook(), null, 2)}
              rows={5}
            />
          </details>
          <div className={styles.actions}>
            <button type="button" className={styles.primary} onClick={saveLook}>
              Save look
            </button>
            <button
              type="button"
          ...
          <div className={styles.secondaryActions}>
            <button
              type="button"
              onClick={() => {
                replaceSettings(family, photoDefaults(photo.id)[family]);
      ...
      <footer className={styles.footer}>
        <span>A working study for mihiir.com</span>
        <a
          href="https://paper.design/blog/retro-print-cmyk-halftone-shader"
          target="_blank"
          rel="noreferrer"
        >
          About Paper’s print filter ↗
        </a>
      </footer>
    </main>
  );
